...
function estimateOrderCostUnits(order: DesiredOrder): bigint {
  const priceBps = Math.round(order.price * 10000);
  return BigInt(priceBps) * BigInt(order.size) * 10_000n; // priceBps / 10_000 * 1e8
}

function estimateBatchCostUnits(orders: DesiredOrder[]): bigint {
  return orders.reduce((sum, order) => sum + estimateOrderCostUnits(order), 0n);
}

function estimateOpenOrderCostUnits(order: OrderRecord): bigint {
  const remaining = order.size > order.filledSize ? order.size - order.filledSize : 0n;
  return BigInt(order.price) * remaining * 10_000n; // priceBps / 10_000 * 1e8
}

function estimateAdditionalAccountBalanceNeeded(desired: DesiredOrder[], existingOrders: OrderRecord[]): bigint {
...
async function requoteMarket(
  ...
  const balanceCheck = await ensureMakerAccountBalance(
    estimateAdditionalAccountBalanceNeeded(desired, myMarketOrders),
  );
  if (!balanceCheck.ok) {
...
// ─── Replenishment — restock depleted books ─────────────────
...
export function isReplenishActive(): boolean {
 ...
 */
export async function replenishDepleted(): Promise<ReplenishResult> {
  ...
  try {
    ...
    let conditionIdMap: Map<string, string>;
    try {
      const unresolved = await getUnresolvedMarkets();
    ...
    const depleted: Array<{ marketId: string; conditionId: string; currentCount: number; desiredCount: number }> = [];
    ...
    for (const [marketId, orders] of ordersByMarket) {
      ...
      const cached = getPrice(conditionId);
      if (!cached || isStale(conditionId, mc.MAX_PRICE_AGE_MS)) continue;
      if (cached.yesPrice === 0 && cached.noPrice === 0) continue;
    ...
    for (const { marketId, conditionId, currentCount, desiredCount } of depleted) {
      ...
      const balanceCheck = await ensureMakerAccountBalance(
        estimateAdditionalAccountBalanceNeeded(desired, ordersByMarket.get(marketId) || []),
      );
      if (!balanceCheck.ok) {
    ...
    log("replenish", "done",
      `Restocked ${result.marketsReplenished}/${result.marketsDepleted} depleted markets, ` +
