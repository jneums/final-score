...
type WsEvent = PriceChangeEvent | BestBidAskEvent | { event_type: string };
...
let onRequote: ((conditionId: string) => void) | null = null;
...
function handleMessage(data: string) {
  ...
  if (event.event_type === "price_change") {
    const pc = event as PriceChangeEvent;
    ...
    const bba = event as BestBidAskEvent;
...
function sendSubscription(assetIds: string[]) {
  if (!ws || !isConnected || assetIds.length === 0) return;
  ...
  for (const id of assetIds) subscribedAssets.add(id);
}
...
export function startWs(requoteCallback: (conditionId: string) => void) {
