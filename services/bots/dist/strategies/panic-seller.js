...
export const panicSeller = {
    ...
    act: async (ctx) => {
        try {
            ...
            ctx.log("panic-seller", "success", `Bought ${size} Yes @ ${yesPrice.toFixed(4)} in ${market.marketId.slice(0, 8)}…`);
            ...
            if (exitResult.ok) {
                ctx.log("panic-seller", "success", `Panic exit: ${size} No @ ${noPrice.toFixed(4)} in ${market.marketId.slice(0, 8)}… — ${exitResult.message}`);
            }
            else {
