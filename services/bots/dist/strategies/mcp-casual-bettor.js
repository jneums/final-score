...
export const mcpCasualBettor = {
    ...
    async act(ctx) {
        ...
        try {
            ...
            if (!pricing) {
                ctx.log("place-order", "skip", `No liquidity for ${outcome} on market ${market.marketId} — skipping`);
                return;
            }
            ctx.log("place-order", "success", `Placing MCP order: ${outcome} @ ${pricing.price} x${pricing.size} on market ${market.marketId}`);
            ...
            if (lowerResp.includes("order_id") ||
