    ...
    act: async (ctx) => {
        try {
            ...
            for (let i = 0; i < numMarkets + 3; i++) { // extra attempts for collisions/skips
                if (placed >= numMarkets)
                    break;
                const market = await getRandomOpenMarket(ctx.candid);
                if (!market)
                    continue;
                if (visited.has(market.marketId))
                    continue;
                ...
                try {
                    ...
                    if (result.ok) {
                        placed++;
                        ctx.log("portfolio-builder", "success", `[${placed}/${numMarkets}] ${size} ${outcome} @ ${price.toFixed(4)} in ${market.marketId.slice(0, 8)}…`);
                    }
                    else {
