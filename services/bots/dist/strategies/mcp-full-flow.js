...
export const mcpFullFlow = {
    ...
    async act(ctx) {
        ...
        try {
            ...
            ctx.log("step-3-place", "success", `Placing order: ${outcome} @ ${pricing.price} x${pricing.size} on ${market.marketId}`);
            const orderResponse = await ctx.mcp.placeOrder(market.marketId, outcome, pricing.price, String(pricing.size));
            const orderId = extractOrderId(orderResponse);
            if (!orderId) {
                ctx.log("step-3-place", "error", `Could not extract order ID from response: ${orderResponse.substring(0, 300)}`);
                return;
            }
            ...
            if (lowerOrders.includes(orderId.toLowerCase()) || lowerOrders.includes("order")) {
                ...
                ctx.log("step-5-verify", "error", `Order ${orderId} not found in open orders: ${ordersResponse.substring(0, 300)}`);
            }
            ...
            if (lowerCancel.includes("cancel") ||
                ...
                ctx.log("step-6-cancel", "error", `Unexpected cancel response: ${cancelResponse.substring(0, 300)}`);
            }
            ...
            ctx.log("step-7-positions", "success", `Positions response: ${positionsResponse.substring(0, 200)}`);
            ...
            if (lowerAccount.includes("balance") || lowerAccount.includes("account")) {
                ...
                ctx.log("step-8-account", "error", `Unexpected account response: ${accountResponse.substring(0, 300)}`);
            }
