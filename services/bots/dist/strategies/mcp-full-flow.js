...
export const mcpFullFlow = {
    ...
    async act(ctx) {
        ...
        try {
            ...
            if (!orderId) {
                ctx.log("step-2-place", "error", `Could not extract order ID from response: ${orderResponse.substring(0, 300)}`);
                return;
            }
            ...
            if (lowerOrders.includes(orderId.toLowerCase()) || lowerOrders.includes("order")) {
                ...
                ctx.log("step-4-verify", "error", `Order ${orderId} not found in open orders: ${ordersResponse.substring(0, 300)}`);
            }
            ...
            if (lowerCancel.includes("cancel") ||
                ...
                ctx.log("step-5-cancel", "error", `Unexpected cancel response: ${cancelResponse.substring(0, 300)}`);
            }
            ...
            ctx.log("step-6-positions", "success", `Positions response: ${positionsResponse.substring(0, 200)}`);
            ...
            if (lowerAccount.includes("balance") || lowerAccount.includes("account")) {
                ...
                ctx.log("step-7-account", "error", `Unexpected account response: ${accountResponse.substring(0, 300)}`);
            }
