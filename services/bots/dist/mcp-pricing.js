 ...
 */
export function parseMarketDetail(response) {
    try {
 ...
 */
export function parseMarketList(response) {
    try {
        const parsed = JSON.parse(response);
        const markets = parsed.markets ?? parsed.data ?? (Array.isArray(parsed) ? parsed : []);
        return markets
            .filter((m) => m.market_id || m.marketId || m.id)
            .map((m) => ({
 ...
 */
export async function getSmartPrice(mcp, marketId, outcome) {
    try {
