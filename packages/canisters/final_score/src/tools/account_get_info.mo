...
module {
  ...
  public func config() : McpTypes.Tool = {
    ...
    description = ?(
      "Returns your custodial Final Score account overview: total internal balance, " #
      "available balance, funds locked in open orders, open positions, and trading stats. " #
  ...
  public func handle(context : ToolContext.ToolContext) : (_args : McpTypes.JsonValue, _auth : ?AuthTypes.AuthInfo, cb : (Result.Result<McpTypes.CallToolResult, McpTypes.HandlerError>) -> ()) -> async () {
    ...
    func(_args : McpTypes.JsonValue, _auth : ?AuthTypes.AuthInfo, cb : (Result.Result<McpTypes.CallToolResult, McpTypes.HandlerError>) -> ()) : async () {
      ...
      let accountBalance = ToolContext.getBalance(context, userPrincipal);
      ...
      ToolContext.makeSuccess(Json.obj([
        ...
        ("account_balance", Json.str(Nat.toText(accountBalance))),
